...
class SimpleAskStreamService
{
    ...
    public function streamToOutputAndCapture(
        array $messages,
        ?string $model = null,
        ...
        foreach ($this->parseSSEStream($response->toPsrResponse()->getBody()) as $event) {
            ...
            if ($event['type'] === 'content' && $event['data']) {
                echo $event['data'];          // → vers le navigateur (le robinet 🦁)
                $this->flush();
                if ($onChunk) {
                    $onChunk($event['data']);  // → vers notre variable pour la base
                }
            }

            // Le reasoning est envoyé au navigateur mais PAS sauvegardé en base
            if ($event['type'] === 'reasoning' && $event['data']) {
                echo "[REASONING]" . $event['data'] . "[/REASONING]";
                $this->flush();
            }
    ...
    private function getSystemPrompt(): array
    {
        ...
        return [
            ...
            'content' => view('prompts.system', [
                ...
                'aboutYou' => $authUser?->about_you,
                'behavior' => $authUser?->behavior,
            ])->render(),
        ];
    }
}
